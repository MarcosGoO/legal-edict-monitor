      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed z-50 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm bg-ink-900 border border-ink-700/60 rounded-xl shadow-xl p-6 focus:outline-none">
          <div className="flex items-start gap-4">
            ...
            <div>
              <Dialog.Title className="text-sm font-semibold text-parchment">{title}</Dialog.Title>
              <Dialog.Description className="text-sm text-ink-600 mt-1">{description}</Dialog.Description>
            </div>
          </div>
          <div className="flex gap-3 mt-6 justify-end">
            <button
              onClick={onCancel}
              className="px-4 py-2 text-sm font-medium text-parchment/70 bg-ink-800 rounded-lg hover:bg-ink-700 transition-colors"
            >
              Cancelar
            </button>
            <button
